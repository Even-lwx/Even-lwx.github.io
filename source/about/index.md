---
title: 关于我
date: 2020-02-23 19:20:33
comments: false
aside: false
---

<div class="career-about">

<p class="career-about__lead">
你好，我是 <strong>if-else</strong>，杭州电子科技大学智能科学与技术专业学生。
我聚焦嵌入式系统、机器人运动控制与计算机视觉，希望用软硬件协同的方法解决真实工程问题。
</p>

## 教育与成果

- 杭州电子科技大学，智能科学与技术本科，2024.9 至今。
- 学业成绩 4.03/5.00，3 次获得创新创业奖学金。
- 第 21 届全国大学生智能汽车竞赛“飞跃雷区”组浙江赛区一等奖（第一名）。
- 浙江省新苗人才计划项目立项，相关实用新型专利已受理。
- 大学英语 CET-6、CET-4，自动化（人工智能）学院科技创新协会副会长。

## 技术方向

<div class="career-about__grid">
<div class="career-about__card">
<h3>MCU 与实时系统</h3>
<p>AURIX、TRAVEO、STM32、ESP32、STC32/8051、CH32、MM32，以及 FreeRTOS 实践。</p>
</div>
<div class="career-about__card">
<h3>控制与感知算法</h3>
<p>串级 PID、模糊 PID、EKF、巴特沃斯滤波，以及 IMU 解算和计算机视觉。</p>
</div>
<div class="career-about__card">
<h3>硬件与机械设计</h3>
<p>单片机与 DCDC 电路设计、PCB Layout 和调试，以及 SolidWorks 零件与装配体建模。</p>
</div>
<div class="career-about__card">
<h3>工程工具与部署</h3>
<p>Keil、IAR、STM32CubeIDE、ADS，以及自动化脚本、上位机、服务器和网页部署实践。</p>
</div>
</div>

## 项目经历

- [四旋翼无人机空地有缆协同系统](/projects/#air-ground-system)：项目负责人，负责无人机端开发、运动控制、视觉识别与路径规划。
- [智能手机协同的桌面级轮足机器人](/projects/#wheeled-leg-robot)：算法负责人，负责 FreeRTOS 软件架构、姿态解算、通信与平衡控制。
- [查看完整项目与技术栈](/projects/)。

## 我如何学习和工作

我倾向于先明确目标和约束，再拆分传感、控制、通信和执行链路，通过日志与实验数据定位问题，最后将方案沉淀为可复用的代码和文档。
这个博客记录的不只是结论，也包括实现过程、失败原因和迭代依据。

## 联系方式

- GitHub：[Even-lwx](https://github.com/Even-lwx)
- QQ 邮箱：[1191228412@qq.com](mailto:1191228412@qq.com)

</div>
